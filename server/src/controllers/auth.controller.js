...
module.exports = (db_pool) => {
    ...
    return {
        signup : async (req, res) => {
            ...
            if (mail.match(regex_mail) == null && mail.length > 0) {
                mail_error = true
            }
            ...
            if (username_error || firstName_error || lastName_error || mail_error || password_error) {
                console.log("[auth.controller]: signup FAILURE", {username_error, firstName_error, lastName_error, mail_error, password_error})
                return res.status(201).send({message: "Cant create user", "username_error" : username_error,
                        "firstName_error" : firstName_error, "lastName_error" : lastName_error, "mail_error" : mail_error, "password_error" : password_error, code: "FAILURE"})
            }
            try {
                ...
                console.log("[auth.controller]: Succesfully created user with id: ", {id : id})
                return res.status(200).send({message: "Successfully created user.", token: token})
            }
            catch (e) {
                if (e.code == 'ER_DUP_ENTRY') {
                    ...
                    return res.status(201).send({message: e.sqlMessage, code: e.code, "username_error" : username_error,
                    ...
                    return res.status(201).send({message: "Data too long", code: e.code, sqlMessage: e.sqlMessage})
                }
                else if (e.code == 'ER_BAD_NULL_ERROR') {
                    console.log("[auth.controller]: signup ER_BAD_NULL_ERROR", e.sqlMessage)
                    return res.status(201).send({message: "data columns cant be null", code: e.code, sqlMessage: e.sqlMessage})
                }
                else {
                    console.log("[auth.controller]: signup ER_BAD_NULL_ERROR", e.sqlMessage)
                    return res.status(500).send({message: 'error in create test user', error: e, code: 'FAILURE'})
                }
            }
        },
